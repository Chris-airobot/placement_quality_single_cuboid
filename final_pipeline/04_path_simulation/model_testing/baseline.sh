...
# Configuration
# Align with comparison_experiment.py output file name
EXPERIMENT_RESULTS_FILE="/home/chris/Chris/placement_ws/src/data/box_simulation/v7/experiments/comparison_results.jsonl"
...
try:
    obj = json.loads(line)
    # Prefer labels.case_index (trial progression), fallback to top-level index if present
    idx = None
    labels = obj.get("labels", {}) if isinstance(obj, dict) else {}
    if isinstance(labels, dict):
        idx = labels.get("case_index")
    if idx is None:
        idx = obj.get("index", -1) if isinstance(obj, dict) else -1
...
run_experiment() {
    ...
    $ISAAC_SIM_PATH -m placement_quality.path_simulation.model_testing.comparison_experiment --/log/level=error --/log/fileLogLevel=error --/log/outputStreamLevel=error
    return $?
}
...
# Ensure resume file exists to satisfy comparison_experiment.py strict resume check
if [ ! -f "$RESUME_FILE" ]; then
    mkdir -p "$(dirname "$RESUME_FILE")"
    echo -n "0" > "$RESUME_FILE"
fi
