...
# Add the model_testing directory to path so its sibling imports resolve
model_testing_dir = os.path.join(current_dir, "model_testing")
if model_testing_dir not in sys.path:
    sys.path.insert(0, model_testing_dir)
...
import os
import datetime 
from model_testing.simulator import Simulator
...
from placement_quality.ycb_simulation.utils.helper import transform_relative_pose
...
def main(use_physics):
    ...
    results_file = os.path.join(DIR_PATH, "diagnosis_results.jsonl")
    ...
    while simulation_app.is_running():
        ...
        env.data_index = DATA_INDEX
        counter += 1
          
        
        if env.state == "SETUP" and counter > 1000:
            # Trace which sample is being processed
            print(f"[Experiment] SETUP for index {env.data_index}")
            env.current_data = env.test_data[env.data_index]
            print(f"current_data: {env.current_data}")
            ...
            grasp_lift_position, grasp_lift_orientation = wpd["L1"][0], wpd["L1"][1]
            place_lift_position, place_lift_orientation = wpd["L2"][0], wpd["L2"][1]
            pre_placement_position, placement_orientation = wpd["P2"][0], wpd["P2"][1]
            placement_position,     placement_orientation    = wpd["C2"][0], wpd["C2"][1]
        ...
        elif env.state == "GRASP_LIFT":
            # Move up from contact to aerial clearance above grasp
            lift1_pose = [grasp_lift_position, grasp_lift_orientation]
            robot_action(env, lift1_pose, "GRASP_LIFT", "PLACE_LIFT")


        elif env.state == "PLACE_LIFT":
            # Aerial move above placement target (lifted)
            preplace_one_pose = [place_lift_position, place_lift_orientation]
            # robot_action(env, preplace_one_pose, "PLACE_LIFT", "PREPLACE")
            robot_action(env, preplace_one_pose, "PLACE_LIFT", "PLACE")

        # elif env.state == "PREPLACE":   
        #     preplace_two_pose = [pre_placement_position, placement_orientation]  # P2
        #     robot_action(env, preplace_two_pose, "PREPLACE", "PLACE")
